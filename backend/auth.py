...
def get_token_from_cookie(request: Request):
    """Dependency to extract the JWT token from the HttpOnly cookie"""
    ...
    # The cookie value will be "Bearer <token>", so we split it
    scheme, _, token_value = token.partition(" ")
    if scheme.lower() != "bearer" or not token_value:
         # Fallback if the cookie just contains the raw token
         return token
    return token_value
